...
class TracyDebugger extends \ProcessWire\WireData implements \ProcessWire\Module, \ProcessWire\ConfigurableModule {
    ...
    public static function getModuleInfo() {
        return array(
            'title' => \ProcessWire\__('Tracy Debugger', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php'),
            'summary' => \ProcessWire\__('Tracy debugger from Nette with many PW specific custom tools.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php'),
            'author' => 'Adrian Jones',
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/tracy-'.self::$tracyVersion.'/src/tracy.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/TD.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 ...
 include_once(\ProcessWire\wire('files')->compile($functionFile,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
            }
        }

        if($this->data['enableShortcutMethods']) {
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/ShortcutMethods.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        }

        // load base panel class
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/BasePanel.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        ...
        if(!is_dir($this->tracyCacheDir)) {
            if(!\ProcessWire\wiremkdir($this->tracyCacheDir)) {
                throw new \ProcessWire\WireException("Unable to create cache path: " . $this->tracyCacheDir);
            }
        }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/CodeProcessor.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
                return;
            }
        }, array('priority' => 9999));
        ...
        if(static::$allowedTracyUser === 'development') {
            ...
            if(in_array('debugMode', static::$showPanels)) {
                ...
                $this->wire()->addHookBefore('PageFinder::getQuery', null, function($event) {
                    $this->timerkey = \ProcessWire\Debug::timer();
                });
                $this->wire()->addHookAfter('PageFinder::getQuery', null, function($event) {
                    $event->setArgument(2, \ProcessWire\Debug::timer($this->timerkey));
            ...
            if($this->wire('input')->cookie->tracyUnhideUnlockFields == 1) {
                $this->wire()->addHookAfter('Field::getInputfield', function(\ProcessWire\HookEvent $event) {
                    ...
                    $inputfield->collapsed = \ProcessWire\Inputfield::collapsedNo;
                });
            }
            ...
            if($this->wire('session')->tracyLoginUrl) {
                $this->wire()->addHookAfter('Session::loginSuccess', function(\ProcessWire\HookEvent $event) {
            ...
            if(in_array('moduleDisabler', static::$showPanels) && $this->wire('config')->debug && $this->wire('config')->advanced) {
                ...
                foreach($this->wire('modules') as $name => $label) {
                    ...
                    if(($flags & \ProcessWire\Modules::flagsAutoload) || ($flags & \ProcessWire\Modules::flagsDisabled)) {
                ...
                if($this->wire('input')->cookie->tracyModulesDisabled) {
                    ...
                    if(!file_exists($this->tracyCacheDir . $this->modulesDbBackupFilename)) {
                        $backup = new \ProcessWire\WireDatabaseBackup($this->tracyCacheDir);
                        ...
                        if(!file_put_contents($this->tracyCacheDir . 'restoremodules.php', $restoreModulesCode, LOCK_EX)) throw new \ProcessWire\WireException("Unable to write file: " . $this->tracyCacheDir . 'restoremodules.php');
                    }
                ...
                foreach(static::$disabableModules as $name) {
                    ...
                    if(in_array($name, static::$disabledModules)) {
                        if(!($flags & \ProcessWire\Modules::flagsDisabled)) {
                            $this->wire('modules')->setFlag($name, \ProcessWire\Modules::flagsDisabled, true);
                            $i++;
                        }
                    }
                    elseif($flags & \ProcessWire\Modules::flagsDisabled) {
                        $this->wire('modules')->setFlag($name, \ProcessWire\Modules::flagsDisabled, false);
                        $i++;
                    }
                }
                    ...
                    if($reason !== true) {
                        if(strpos($reason, 'Fieldtype') !== false) {
                            foreach($this->wire('fields') as $field) {
                                $fieldtype = \ProcessWire\wireclassname($field->type, false);
                                if($fieldtype == $class) {
                                    foreach($this->wire('templates') as $template) {
                                        if(!$template->hasField($field)) continue;
                                        $template->fields->remove($field);
            ...
            if($this->wire('input')->post->clearEmailSent || $this->wire('input')->get->clearEmailSent) {
                ...
                if (!isset($removed) || !$removed) {
                    $this->wire()->error( \ProcessWire\__('No file to remove'));
                }
                else {
                    $this->wire()->message(\ProcessWire\__("email-sent file deleted successfully"));
                    $this->wire('session')->redirect(str_replace(array('?clearEmailSent=1', '&clearEmailSent=1'), '', $this->wire('input')->url(true)));
                }
            }

            if(file_exists($emailSentPath)) {
                $this->wire()->warning('Tracy Debugger "Email Sent" flag has been set. <a href="'.$this->wire('input')->url(true).($this->wire('input')->queryString() ? '&' : '?').'clearEmailSent=1">Clear it</a> to continue receiving further emails', \ProcessWire\Notice::allowMarkup);
            }
        ...
        $this->wire()->addHookAfter('ProcessWire::trackException', function($event) {
            ...
            if(!$exception instanceof \ProcessWire\WireException && $event->arguments(1) !== false) {
                throw $exception;
            }
        });
        ...
        if($this->wire('input')->post->deleteTracyLogs) {
            \ProcessWire\wirermdir($logFolder, true);
        }

        // if Tracy log folder doesn't exist, create it now
        if(!is_dir($logFolder)) \ProcessWire\wiremkdir($logFolder);
                ...
                if(!$this->wire('config')->ajax && (in_array('console', static::$showPanels) || in_array('templateResources', static::$showPanels))) {
                    ...
                    $this->wire()->addHookBefore('TemplateFile::render', function($event) {
                        ...
                        if(!$event->object->hanna) {
                            $event->object->setAppendFilename(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/GetTemplateResources.php');
                        }
                    });
                }
            }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/GetFileDetails.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
                return;
            }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/GetPageById.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
                return;
            }
            ...
            if(static::$allowedSuperuser || self::$validLocalUser || self::$validSwitchedUser) {
                if($this->wire('input')->post->fileEditorFilePath) {
                    ...
                    if($this->wire('input')->post->tracyTestTemplateCode) {
                        if(!file_put_contents($this->tempTemplateFilename, $rawCode, LOCK_EX)) throw new \ProcessWire\WireException("Unable to write file: " . $this->tempTemplateFilename);
                    ...
                    if($this->wire('input')->post->tracyTestFileCode || $this->wire('input')->post->tracySaveFileCode || $this->wire('input')->post->tracyChangeTemplateCode) {
                        if($this->wire('input')->post->fileEditorFilePath != '' && strpos($this->wire('input')->post->fileEditorFilePath, '..') === false) {
                            ...
                            if(!is_dir($cachePath)) if(!\ProcessWire\wiremkdir(pathinfo($cachePath, PATHINFO_DIRNAME), true)) {
                                throw new \ProcessWire\WireException("Unable to create cache path: $cachePath");
                            }
                            copy($filePath, $cachePath);

                            if(!file_put_contents($filePath, $rawCode, LOCK_EX)) throw new \ProcessWire\WireException("Unable to write file: " . $filePath);
        ...
        foreach(static::$showPanels as $panel) {
            ...
            if(file_exists(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/panels/'.$panelName.'.php')) {
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/panels/'.$panelName.'.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
            }
            else {
                // external panels
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/ConsoleSnippets.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
            return;
        }
    ...
    /**
     * Hook after \ProcessWire\Page::render()
     *
     * Add the User bar
     *
    ...
    protected function addUserBar($event) {
        $userBar = '';
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/user-bar/UserBar.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        $return = $event->return;
    ...
    /**
     * Hook before \ProcessWire\WireMail::send
     *
    ...
    /**
     * Hook after \ProcessWire\Module::install, uninstall, or version change
     *
    ...
    /**
     * Hook after \ProcessWire\Pages::added
     *
    ...
    /**
     * Replace auto appended / prepended Template \ProcessWire\files(_main.php, _init.php etc) with "dev" versions
     *
    ...
    private static function userHasPermission($permission) {
        foreach(\ProcessWire\wire('user')->roles as $role) {
 ...
 require_once(\ProcessWire\wire('files')->compile($consoleCodeFile,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        }
        else {
     ...
     * @return void
     */
    public function logRequests(\ProcessWire\HookEvent $event) {
    ...
    private function _getRequestLoggerData($id, $all = false) {
        ...
        // it's a logged request ID
            ...
            $data = \ProcessWire\wire('cache')->get("tracyRequestLogger_id_$id*");
    ...
    public static function templateVars($vars) {
        ...
        foreach(\ProcessWire\wire('config')->version >= 2.8 ? \ProcessWire\wire('fuel') : \ProcessWire\wire()->fuel as $key => $value) {
    ...
    */
    ...
    public static function generatePanelSettingsLink($hashLink) {
        ...
        <span class="tracy-panel-settings-link">
            <a title="Panel Settings" href="'.\ProcessWire\wire('config')->urls->admin.'module/edit?name=TracyDebugger#'.$hashLink.'">
    ...
    public static function createEditorLink($file, $line, $linkText, $title = null) {
        ...
        elseif(self::getDataValue('localRootPath') != '') $file = str_replace(\ProcessWire\wire('config')->paths->root, self::getDataValue('localRootPath'), $file);
        return '<a '.($title ? 'title="'.$title.'"' : '').' href="'.$file.'">'.$linkText.'</a>';
    }


    public static function removeCompilerFromPath($path) {
        $compilerCachePath = isset(\ProcessWire\wire('config')->fileCompilerOptions['cachePath']) && \ProcessWire\wire('config')->fileCompilerOptions['cachePath'] != '' ? \ProcessWire\wire('config')->fileCompilerOptions['cachePath'] : \ProcessWire\wire('config')->paths->cache . 'FileCompiler/';
        return str_replace($compilerCachePath, \ProcessWire\wire('config')->paths->root, $path);
    }
    ...
    public static function getDataValue($property) {
        if(!is_string(self::$_data->$property) || is_array(self::$_data->$property) || is_int(self::$_data->$property) || $property == 'consoleCodePrefix') {
    ...
    public static function allowedTracyUser() {
        ...
        elseif(\ProcessWire\wire('session')->tracyUserSwitcherId && (isset($userSwitchSession[\ProcessWire\wire('session')->tracyUserSwitcherId]) && $userSwitchSession[\ProcessWire\wire('session')->tracyUserSwitcherId] > time())) {
    ...
    */
    public static function inputHttpUrl($withQueryString = false) {
        return \ProcessWire\wire('input')->scheme() . '://' . \ProcessWire\wire('config')->httpHost . self::inputUrl($withQueryString);
    }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/PwApiData.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    public function __destruct() {
        ...
        // this is here instead of \ProcessWire\ProcessWire::finished because this works if test version has fatal error
    ...
    public function getModuleConfigInputfields(array $data) {
        ...
        $fieldset->label = \ProcessWire\__(' ', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $fieldset->value = '
        ...
        $f->label = \ProcessWire\__('Quick links', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->value = '<ul></ul>';
        $wrapper->add($f);
        ...
        $fieldset->label = \ProcessWire\__('Main setup', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'enabled');
        $f->label = \ProcessWire\__('Enable Tracy Debugger', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Uncheck to completely disable all Tracy Debugger features.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Use Native PHP Session', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to use native PHP session instead of temporary files.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Use this if you are having problems with Tracy\'s AJAX and Redirect bars not showing. IMPORTANT: Do not use this option together with the SessionHandlerDB module.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        ...
        $f->label = 'Output mode';
        $f->description = \ProcessWire\__('The DETECT option automatically switches from DEVELOPMENT to PRODUCTION mode based on whether the IP of the site is publicly accessible or not.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('In PRODUCTION mode, all errors and dumps etc are logged to file. Nothing is displayed in the browser. In DEVELOPMENT mode, the debug bar is displayed for authorized users - superusers and those with the `tracy-debugger` permission. All other users will be forced into PRODUCTION mode, regardless of this setting.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        $f->required = true;
        ...
        $fieldset->label = \ProcessWire\__('Access permission', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'superuserForceDevelopment');
        $f->label = \ProcessWire\__('Force superusers into DEVELOPMENT mode', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to force DEVELOPMENT mode for superusers even on live sites.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('By default, the Output Mode setting\'s DETECT option will force a site into PRODUCTION mode when it is live, which hides the DebugBar and sends errors and dumps to log files. However, with this checked, superusers will always be in DEVELOPMENT mode.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Force guest users into DEVELOPMENT mode on localhost', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to force DEVELOPMENT mode for guests when server detected as localhost.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('By default, guest users will always be in PRODUCTION mode (no debug bar). However, with this checked, they will always be in DEVELOPMENT mode on localhost.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        ...
        $f->label = \ProcessWire\__('Force isLocal check to always return true', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to force isLocal.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('WARNING - Only use this if you know what you are doing! You will expose debug info to guest users on a live site when combined with the previous setting.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Restrict non-superusers', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('IP Address that non-superusers need to use TracyDebugger. Enter IP address or a PCRE regular expression to match IP address of user, eg. /^123\.456\.789\./ would match all IP addresses that started with 123.456.789.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        $f->notes = \ProcessWire\__('Non-superusers are already blocked unless they have the `tracy-debugger` permission. But once a user has been given the permission, this option restricts access to the listed IP address. Highly recommended for debugging live sites that you have manually set into DEVELOPMENT mode.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        if($data['ipAddress']) $f->attr('value', $data['ipAddress']);
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'restrictSuperusers');
        $f->label = \ProcessWire\__('Restrict superusers', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('If checked, only superusers with the `tracy-debugger` permission will have access to Tracy.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('Miscellaneous', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'strictMode');
        $f->label = \ProcessWire\__('Strict mode', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to enable strict mode which displays notices and warnings like errors.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Strict mode AJAX only', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to enable strict mode only for AJAX calls.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Because Tracy intercepts notices and warnings, these will no longer be returned with the AJAX response which may result in a "success" response, rather than "failure". Notices and warnings from an AJAX call will be displayed in the AJAX bar\'s Errors panel, but you still might prefer this option as it provides a more prominent indication of failure.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->label = \ProcessWire\__('Force scream', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to force "scream" of mode which disables the @ (silence/shut-up) operator so that notices and warnings are no longer hidden.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This is disabled when Strict Mode is enabled because of a bug? [https://forum.nette.org/en/25569-strict-and-scream-modes-together](https://forum.nette.org/en/25569-strict-and-scream-modes-together) in the core Tracy package.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->description = \ProcessWire\__('Shows the location of dump() and barDump() calls.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('LOCATION_SOURCE adds tooltip with path to the file, where the function was called.'."\n".'LOCATION_LINK adds a link to the file which can be opened directly.'."\n".'LOCATION_CLASS adds a tooltip to every dumped object containing path to the file, in which the object\'s class is defined.');
        ...
        $f->label = \ProcessWire\__('Use debugInfo() magic method', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('If a `__debugInfo()` method has been defined, it will be used instead of dumping the full object.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This results in a smaller, cleaner dump, but you may miss some information. You can override this with the `debugInfo => true/false` option when calling `d()`, `bd()`, `fl()`, etc. Note that this also affects the output in the Request Info panel\'s Field List & Values section.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Maximum nesting depth', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Set the maximum nesting depth of dumped arrays and objects.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 3. Warning: making this too large can slow your page load down or even crash your browser.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Maximum string length', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Set the maximum displayed strings length.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 150', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        ...
        $f->label = \ProcessWire\__('Maximum number of items', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Set the maximum number of items displayed.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 100', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Collapse top array/object', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Set how big are collapsed', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 14', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Collapse array/object', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Set how big are collapsed', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 7', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Maximum number of AJAX rows in debug bar', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('After number is exceeded, the first one will be recycled.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 3. Note that you will need to do a hard browser reload for this setting to take effect.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Reserved memory size', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('If you are getting memory exhaustion errors on Tracy\'s bluescreen, try increasing this value.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 500000', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Send data to FireLogger', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('When checked, certain errors and `fl()` calls will be sent to FireLogger in the browser console.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('If you are running on nginx and don\'t have access to adjust `fastcgi_buffers` and `fastcgi_buffer_size` settings, you may want to uncheck this to avoid 502 bad gateway errors because of `upstream sent too big header while reading response header from upstream` issues.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->attr('checked', $data['showFireLogger'] == '1' ? 'checked' : '');
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'referencePageEdited');
        $f->label = \ProcessWire\__('Reference page being edited', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('When editing a page in the admin, the Request Info Panel will show details of the page being edited, and the Consol Panel will assign the $page variable to the page being edited.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Highly recommended unless you have a reason not to do this.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Open links in new tab', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Makes links open in a new browser tab.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This is used by links such as those in the Captain Hook and API Explorer panels.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('Error logging', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'logSeverity');
        $f->label = 'Log severity';
        $f->description = \ProcessWire\__('If you want Tracy to log PHP errors like E_NOTICE or E_WARNING with detailed information (HTML report), set them here.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('These only affect log file content, not onscreen debug info.');
        $f->columnWidth = 25;
        ...
        $f->label = \ProcessWire\__('Email errors "From"', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Receive emails from this address when an error occurs.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 25;
        ...
        $f = $this->wire('modules')->get("InputfieldEmail");
        $f->attr('name', 'email');
        $f->label = \ProcessWire\__('Email errors "To"', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Receive emails at this address when an error occurs.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 25;
        ...
        $f->label = \ProcessWire\__('Clear "email sent" flag', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check and save settings to remove the "email-sent" file so that you will start receiving new error emails.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 25;
        $fieldset->add($f);
        ...
        $fieldset->label = \ProcessWire\__('Debug bar and panels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'showDebugBar');
        $f->label = \ProcessWire\__('Show debug bar', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Show the debug bar.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('No debug bar in ...', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('No debug bar in selected frontend templates', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Disable the debug bar on pages with the selected templates.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('No debug bar in selected backend templates', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Disable the debug bar when editing pages with the selected templates.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        foreach($this->wire('templates') as $t) {
            $f->addOption($t->id, $t->name);
        }
        ...
        $f->label = \ProcessWire\__('Hide debug bar by default', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Hide the debug bar by default on page load.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This results in the bar being hidden (unless an error is reported), and replaced with a small "show bar" &#8689; icon.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Show panel labels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Show the labels next to each panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Unchecking this will make the debugger bar much more compact.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Bar Position', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('You will need to do a hard reload in your browser for these changes to take effect.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Starting z-index for panels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Adjust if you find panels are below/above elements that you don\'t want.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 100', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Frontend panels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Determines which panels are shown in the Debug Bar on the frontend. Sort to match order of panels in Debugger Bar.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Backend panels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Determines which panels are shown in the Debug Bar on the backend. Sort to match order of panels in Debugger Bar.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Note that '.implode(', ', static::$hideInAdmin).' are intentionally missing from this list because they have no use in the backend.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        foreach(static::$allPanels as $name => $label) {
        ...
        $f->label = \ProcessWire\__('Disabled panels for restricted users', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check the panels that should NOT be shown to users with the `tracy-restricted-panels` role or permission.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Unchecked panels will still only be shown to restricted users if they are selected in the Front/Back-end options above.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->optionColumns = 3;
        foreach(static::$allPanels as $name => $label) {
            $f->addOption($name, $label);
        }
        ...
        $fieldset->label = \ProcessWire\__('Panel Selector panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'nonToggleablePanels');
        $f->label = \ProcessWire\__('Non-toggleable panels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Selected panels will NOT be toggleable in the Panel Selector.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->optionColumns = 3;
        foreach(static::$allPanels as $name => $label) {
            $f->addOption($name, $label);
        }
        ...
        $f->label = \ProcessWire\__('Add Disable/Enable (Toggler) Tracy button to Panel Selector.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This allows you to disable/enable Tracy from the Panel Selector.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Editor links', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldText");
        $f->attr('name', 'editor');
        $f->label = \ProcessWire\__('Editor protocol handler', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Sets the Tracy `Debugger::$editor` variable. Enter the appropriate address to open your code editor of choice.'."\n".'This approach only works for OSX. For more instructions on Windows and Linux alternatives, [read here](https://pla.nette.org/en/how-open-files-in-ide-from-debugger).'."\n\n**Protocol handler helpers**\n[VSCode](https://github.com/shengyou/vscode-handler)\n[Sublime Text](https://github.com/saetia/sublime-url-protocol-mac)\n[PHP Storm](https://github.com/aik099/PhpStormProtocol)\n".' For other editors/IDEs, Google "protocol handler editorname".', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__("`vscode://file/%file:%line`\n`subl://open/?url=file://%file&line=%line`\n`phpstorm://open?file=%file&line=%line`\n Initially configured for VSCode - change to work with your favorite editor.", \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Local root path', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Maps editor links from site root to local path.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('An example path on MacOS might be: /Users/myname/Sites/sitefolder/', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Use online editor for links', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This will open links in an online editor (Tracy File Editor or ProcessFileEdit) instead of your code editor.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('You can choose which editor after selecting at least one of these.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Online editor', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Which online editor to use: Tracy File Editor or [ProcessFileEdit (Files Editor)](http://modules.processwire.com/modules/process-file-edit/).', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        if($this->wire('modules')->isInstalled('ProcessFileEdit')) {
            $f->notes = \ProcessWire\__('Process File Edit is installed, but you may want to adjust its [settings]('.$this->wire('config')->urls->admin.'module/edit?name=ProcessFileEdit)', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        }
        else {
            $f->notes = \ProcessWire\__('Process File Edit is NOT installed - you need to install it for the option to appear.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        }
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Force editor links to use Tracy File Editor', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Even if neither of the "Use Online Editor for Links" options are checked, if the File Editor Panel is enabled, all links will be sent to it.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__("RECOMMENDED: This is a handy option if you generally want links to use your code editor, but want the option to use the File Editor occasionally.\nYou can enable it (once or sticky) from the Panel selector on the debug bar without the need to change the above settings.", \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Console panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldRadios");
        $f->attr('name', 'snippetsPath');
        $f->label = \ProcessWire\__('Snippets path', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Directory where snippets will be saved to / loaded from', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: templates', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->label = \ProcessWire\__('Maximum number automatically named backups', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('The maximum number of automatically named backups that will be retained before pruning the oldest.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 25.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->attr('value', $data['consoleBackupLimit']);
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldTextarea");
        $f->attr('name', 'consoleCodePrefix');
        $f->label = \ProcessWire\__('Code prefix', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Code block that should be added to each snippet stored on disk.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->noTrim = true;
        ...
        $fieldset->label = \ProcessWire\__('File Editor panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $fieldset->description = \ProcessWire\__("These settings don't affect links opened via the Editor Protocol Handler. They only affect browsing directly from the File Editor folder/file selector sidebar.");
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldMarkup");
        $f->attr('name', 'fileEditorNote');
        $f->showIf = "useOnlineEditor.count>0, onlineEditor!=processFileEdit";
        $f->value = \ProcessWire\__('Because you are using Tracy File Editor for the editor links, the File Editor Panel will be enabled, even if you don\'t have it selected in the Frontend/Backend Panel selections above.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldSelect");
        $f->attr('name', 'fileEditorBaseDirectory');
        $f->label = \ProcessWire\__('Base directory', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('A more specific selection results in better performance in the File Editor Panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Allowed extensions', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Comma separated list of extensions that can be opened in the editor. Fewer extensions results in better performance in the File Editor Panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Initially configured for: php, module, js, css, txt, log, htaccess', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Excluded directories', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Comma separated list of directories that will be excluded from the tree.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Initially configured for: site/assets', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Code editor settings', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $fieldset->description = \ProcessWire\__("These settings apply to the Console panel and the File Editor panel", \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldSelect");
        $f->attr('name', 'aceTheme');
        $f->label = \ProcessWire\__('Theme', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        $themes = array();
        foreach (new \DirectoryIterator(\ProcessWire\wire('config')->paths->$this.'scripts/ace-editor/') as $file) {
            if($file->isFile()) {
        ...
        $f->label = \ProcessWire\__('Font size', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('In pixels.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('If you change the font size you must also set the "Line height" to an appropriate value relative to the font size.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        $f->required = true;
        ...
        $f->label = \ProcessWire\__('Line height', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('In pixels.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('You must set this to an appropriate value relative to the font size.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        $f->required = true;
        ...
        $f->label = \ProcessWire\__('Show Invisibles', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Show invisible characters like spaces, CR, and LF.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        $f->addOption(1, 'True');
        $f->addOption(0, 'False');
        $f->required = true;
        ...
        $f->label = \ProcessWire\__('Tab size', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Number of spaces (or equivalent tab width).', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        $f->required = true;
        ...
        $f->label = \ProcessWire\__('Use Soft Tabs', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Uses spaces rather than actual (hard) tabs.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        $f->addOption(1, 'True');
        $f->addOption(0, 'False');
        $f->required = true;
        ...
        $f->label = \ProcessWire\__('ProcessWire autocompletions', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Adds PW methods, properties, and page fields to the editor autocompletions.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This add approximately 200KB to the payload of the Console and File Editor panels.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Show description', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This will show the first line from the doc comment as a note connected to autocomplete matches.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Unchecking this reduces the autocomplete payload to approximately 100KB.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Custom snippets URL', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Link to snippets file for use in Console and File Editor panels. Can be local or remote, eg. a Github Gist file.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('If Github gist, you must use a service such as [https://rawgit.com/](https://rawgit.com/)', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('ProcessWire Info panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'processwireInfoPanelSections');
        $f->label = \ProcessWire\__('Panel sections', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Which sections to include in the ProcessWire Info panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Custom links', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Choose pages you would like links to.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('To provide links to items under the Setup menu, navigate to Admin > Setup >', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Show icon labels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Shows labels next to each icon for the two "Links" sections.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Nice for clarity, but takes up more space.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        if($this->wire('modules')->isInstalled('ProcessTracyAdminer')) {
            ...
            $fieldset->label = \ProcessWire\__('Adminer panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
            $wrapper->add($fieldset);
        ...
        $fieldset->label = \ProcessWire\__('API Explorer panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'apiExplorerShowDescription');
        $f->label = \ProcessWire\__('Show description', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This will show the first line from the doc comment in its own column.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Toggle method doc comment', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This will toggle the entire doc comment block if you click on the method column.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This significantly increases the size of this panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Module classes', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Select module classes that you also want diplayed.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('These options will significantly increase the size of this panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Captain Hook panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);
        ...
        $fieldset->label = \ProcessWire\__('Request Info panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'requestInfoPanelSections');
        $f->label = \ProcessWire\__('Panel sections', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Which sections to include in the Request Info panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        $f->notes = \ProcessWire\__('The three "Object" options will significantly increase the size of this panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->label = \ProcessWire\__('Show image thumbnails in Field List & Values section', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This will load all image thumbnails for the page, along with the dimensions & size details.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('This can significantly increase the size of this panel and rendering time if the page has lots of images.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('Debug Mode panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'debugModePanelSections');
        $f->label = \ProcessWire\__('Panel sections', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Which sections to include in the Debug Mode panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Show debug mode tools even if $config->debug = false.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('If checked, the debug tools will be displayed regardless of whether debug mode is enabled.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('Diagnostics panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'diagnosticsPanelSections');
        $f->label = \ProcessWire\__('Panel sections', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Which sections to include in the Diagnostics panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('The "Filesystem Files" option may significantly increase the generation time for this panel.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Dumps panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldAsmSelect");
        $f->attr('name', 'dumpPanelTabs');
        $f->label = \ProcessWire\__('Dump Tabs', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Select and order the tabs you want when dumping ProcessWire objects via dump/barDump methods.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('The first item will be open by default.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Validator panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldText");
        $f->attr('name', 'validatorUrl');
        $f->label = \ProcessWire\__('Validator URL', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Enter the URL for the validation service.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: https://html5.validator.nu/', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('TODO panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldTextarea");
        $f->attr('name', 'todoIgnoreDirs');
        $f->label = \ProcessWire\__('Ignore directories', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Comma separated list of terms used to match folders to be ignored when scanning for ToDo items.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: git, svn, images, img, errors, sass-cache, node_modules', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Allowed extensions', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Comma separated list file extensions to be scanned for ToDo items.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: php, module, inc, txt, latte, html, htm, md, css, scss, less, js', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Scan site modules', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to allow the ToDo to scan the /site/modules directory. Otherwise it will only scan /site/templates.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Not recommended unless you are a regular module developer.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Scan site assets', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Check to allow the ToDo to scan the /site/assets directory. Otherwise it will only scan /site/templates.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('If you check this, you should add files, logs, cache, sessions and other relevant terms to the `Ignore Directories` field.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('ProcessWire and Tracy Log panels', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldInteger");
        $f->attr('name', 'numLogEntries');
        $f->label = \ProcessWire\__('Number of log entries', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Set the number of log entries to be displayed for the Tracy and ProcessWire log viewer panels.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Default: 100', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Template Resources panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'variablesShowPwObjects');
        $f->label = \ProcessWire\__('Show content of ProcessWire objects', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Shows the full ProcessWire object contents, rather than arrays of values. Only recommended for specific debugging purposes.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Checking this will significantly increase the size of this panel if you have any variables set to ProcessWire objects.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Links panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldTextarea");
        $f->attr('name', 'linksCode');
        $f->label = \ProcessWire\__('Links code', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('One link per line. Optionally add label for link.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('eg. https://www.google.com | Google Search', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Custom PHP panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldTextarea");
        $f->attr('name', 'customPhpCode');
        $f->label = \ProcessWire\__('Custom PHP code', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Use this PHP code block to return any output you want.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('eg. return \'<a href="https://developers.google.com/speed/pagespeed/insights/?url=\'.$page->httpUrl.\'">Google PageSpeed</a>\';', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->notes = \ProcessWire\__('Note: this is just an extra Submit button to make iterations of the customPHP code quicker to make. It works exactly the same as the main ones on this page.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $fieldset->add($f);

        // User Switcher Panel
        $fieldset = $this->wire('modules')->get("InputfieldFieldset");
        $fieldset->attr('name+id', 'userSwitcherPanel');
        $fieldset->label = \ProcessWire\__('User Switcher panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldMarkup");
        $f->label = \ProcessWire\__(' ', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->label = \ProcessWire\__('Selector', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Use this to determine which users will be available.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        if($data['userSwitcherSelector']) $f->attr('value', $data['userSwitcherSelector']);
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldAsmSelect");
        $f->attr('name', 'userSwitcherRestricted');
        $f->label = \ProcessWire\__('Excluded Roles', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Users with selected roles will not be available from the list of users to switch to.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Included Roles', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Only users with these selected roles will be available from the list of users to switch to. If none selected, then all will be available unless the excluded roles is populated.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        $f->setAsmSelectOption('sortable', false);
        foreach($this->wire('roles') as $role) {
            $f->addOption($role->id, $role->name);
        ...
        $fieldset->label = \ProcessWire\__('Request Logger panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'requestMethods');
        $f->label = \ProcessWire\__('Request methods', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Which request methods to log.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('It may be useful to disable GET so that normal page visits are ignored.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->addOption('GET');
        $f->addOption('POST');
        $f->addOption('PUT');
        $f->addOption('DELETE');
        ...
        $f->label = \ProcessWire\__('Maximum number of logged requests', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Number of requests to be kept for each page.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        ...
        $f->label = \ProcessWire\__('Output type', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This determines whether the logged data is returned as an object or an array.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $fieldset->label = \ProcessWire\__('Server type indicator', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'styleWhere');
        $f->label = \ProcessWire\__('Where', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Add indicator based on server IP address and/or subdomain.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 30;
        $f->addOption('backend', 'Backend');
        $f->addOption('frontend', 'Frontend');
        ...
        $f->label = \ProcessWire\__('Indicator colors', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "styleWhere.count>0";
        $f->description = \ProcessWire\__('Use "type|#color" entries to define indicator styling for each server type. "local" is determined by IP address. Other types are detected based on their existence in subdomain or TLD (eg. dev.mysite.com or mysite.dev), dependant on whether you include the period after (dev.﹡) or before (﹡.dev).', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__("**DEFAULT**\nlocal|#FF9933\n*.local|#FF9933\ndev.*|#FF9933\n*.test|#FF9933\nstaging.*|#8b0066\n*.com|#009900", \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 70;
        ...
        $f->label = \ProcessWire\__('Indicator type', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "styleWhere.count>0";
        $f->description = \ProcessWire\__('Choose custom if you want complete control', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('You will need to do a hard reload in your browser for these changes to take effect.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->label = \ProcessWire\__('Custom indicator CSS', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "styleWhere.count>0, styleAdminType='custom'";
        $f->description = \ProcessWire\__('Use [color] and [type] shortcodes to add indicator based on server type.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__("**DEFAULT**\nbody::before {\n&nbsp;&nbsp;&nbsp;&nbsp;content: \"[type]\";\n&nbsp;&nbsp;&nbsp;&nbsp;background: [color];\n&nbsp;&nbsp;&nbsp;&nbsp;position: fixed;\n&nbsp;&nbsp;&nbsp;&nbsp;left: 0;\n&nbsp;&nbsp;&nbsp;&nbsp;bottom: 100%;\n&nbsp;&nbsp;&nbsp;&nbsp;color: #ffffff;\n&nbsp;&nbsp;&nbsp;&nbsp;width: 100vh;\n&nbsp;&nbsp;&nbsp;&nbsp;padding: 0;\n&nbsp;&nbsp;&nbsp;&nbsp;text-align: center;\n&nbsp;&nbsp;&nbsp;&nbsp;font-family: sans-serif;\n&nbsp;&nbsp;&nbsp;&nbsp;font-weight: 600;\n&nbsp;&nbsp;&nbsp;&nbsp;text-transform: uppercase;\n&nbsp;&nbsp;&nbsp;&nbsp;transform: rotate(90deg);\n&nbsp;&nbsp;&nbsp;&nbsp;transform-origin: bottom left;\n&nbsp;&nbsp;&nbsp;&nbsp;z-index: 999999;\n&nbsp;&nbsp;&nbsp;&nbsp;font-family: sans-serif;\n&nbsp;&nbsp;&nbsp;&nbsp;font-size: 11px;\n&nbsp;&nbsp;&nbsp;&nbsp;height: 13px;\n&nbsp;&nbsp;&nbsp;&nbsp;line-height: 13px;\n&nbsp;&nbsp;&nbsp;&nbsp;pointer-events: none;\n}\n", \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 70;
        ...
        $fieldset->label = \ProcessWire\__('User dev template', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'userDevTemplate');
        $f->label = \ProcessWire\__('Enable user dev template', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('If user has a permission named to match the page template with the set suffix, the page will be rendered with that template file rather than the default.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('User dev template suffix', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Template file suffix. eg "dev" will render the homepage with the "home-dev.php" template file if the user has a matching permission (prefixed with "tracy"), eg. `tracy-home-dev`. You can also use `tracy-all-dev` to enable for all templates if the files exist.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('User Bar', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'showUserBar');
        $f->label = \ProcessWire\__('Show user bar', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('This bar is shown to logged in users without permission for the Tracy debug bar (typically all non-superusers).', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Show user bar for Tracy users', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "showUserBar=1";
        $f->description = \ProcessWire\__('Also show the bar to users with Tracy debug bar permission.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Be sure to position this bar somewhere other than bottom right so it doesn\'t conflict with the debug bar.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        $f->showIf = "showUserBar=1";
        ...
        $f->label = \ProcessWire\__('Features', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "showUserBar=1";
        $f->description = \ProcessWire\__('Determines which features are shown on the User Bar.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('The Page Versions function requires that the user has the `tracy-page-versions` permission.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Left / Right', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "showUserBar=1";
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Background color', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "showUserBar=1";
        $f->description = \ProcessWire\__('Leave blank for transparent/none', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('eg. #FFFFFF', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $f->label = \ProcessWire\__('Background opacity', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "showUserBar=1";
        $f->notes = \ProcessWire\__('0 to 1', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        ...
        $f->label = \ProcessWire\__('Icon color', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->showIf = "showUserBar=1";
        $f->notes = \ProcessWire\__('eg. #666666', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 33;
        ...
        $fieldset->label = \ProcessWire\__('Method shortcuts', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'enableShortcutMethods');
        $f->label = \ProcessWire\__('Enable shortcut methods', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Uncheck to not define any of the shortcut methods. If you are not going to use these in your templates, unchecking means that they will not be defined which may reduce possible future name clashes. If in doubt, uncheck and use the full methods:'."\n".'TD::addBreakpoint()'."\n".'TD::barDump()'."\n".'TD::barDumpBig()'."\n".'TD::debugAll()'."\n".'TD::dump()'."\n".'TD::dumpBig()'."\n".'TD::fireLog()'."\n".'TD::log()'."\n".'TD::templateVars()'."\n".'TD::timer()', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('If this, or one of the shortcut methods is not enabled, but is called in your templates, all users will get a "call to undefined function" fatal error, so please be aware when using the shortcut methods in your templates if they are not enabled here.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Enabled shortcuts', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Uncheck any shortcuts/aliases to methods that you do not want available.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Useful if any of these functions/methods are defined elswhere in your site and you are getting a "previously declared" fatal error.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
 ...
 include_once(\ProcessWire\wire('files')->compile($path,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    public function ___upgrade($fromVersion, $toVersion) {
        ...
        if(isset($this->data['snippets'])) {
            ...
            if(!file_exists($snippetsPath)) \ProcessWire\wiremkdir($snippetsPath, true);
