...
class RequestInfoPanel extends BasePanel {
            ...
            if($this->wire('input')->get('id') && $this->wire('page')->process == 'ProcessField') {
                ...
                if($field) {
                    if(method_exists($field, 'getExportData')) {
                        ...
                        $fieldExportCode .= $this->wire('sanitizer')->entities(\ProcessWire\wireencodejson($fieldExportData, true, true));
                    }
                    $fieldExportCode .= '</pre>';
                }
            }
            ...
            if($this->wire('input')->get('id') && $this->wire('page')->process == 'ProcessTemplate') {
                ...
                if($template) {
                    ...
                    if(method_exists($template, 'getExportData')) {
                        ...
                        $templateCode .= $this->wire('sanitizer')->entities(ltrim(rtrim(ltrim(str_replace(':', ' =>', \ProcessWire\wireencodejson($templateExportData, true, true)), "{"), "}"), "\n"));
                    }
                    // older version of PW that doesn't have getExportData() method
                    else {
                        $templateCode .= "\t'type' => '" . (string)$template->getInputfield(new \ProcessWire\NullPage()) . "',\n";
            ...
            if($this->wire('input')->get('id') && $this->wire('page')->process == 'ProcessTemplate') {
                ...
                if($template) {
                    if(method_exists($template, 'getExportData')) {
                        ...
                        $templateExportCode .= $this->wire('sanitizer')->entities(\ProcessWire\wireencodejson($templateExportData, true, true));
                    }
    ...
    private function imageDetails($p, $f) {
        ...
        if($f->type instanceof FieldtypeRepeater) {
						...
						if($repeaterValue instanceof \ProcessWire\Page) $repeaterValue = array($repeaterValue); //support for FieldtypeFieldsetPage
						if($repeaterValue) {
