...
class TD extends TracyDebugger {
    ...
    public static function sql($selector) {
        return \ProcessWire\wire('pages')->getPageFinder()->find(new \ProcessWire\Selectors($selector), array('returnVerbose' => true,'returnQuery' => true))->getDebugQuery();
    }
    ...
    private static function dumpToBar($var, $title = NULL, array $options = NULL, $echo = false) {
        ...
        if((self::tracyUnavailable() && \TracyDebugger::getDataValue('recordGuestDumps')) || (isset(\TracyDebugger::$showPanels) && in_array('dumpsRecorder', \TracyDebugger::$showPanels))) {
            $dumpsFile = \ProcessWire\wire('config')->paths->cache . 'TracyDebugger/dumps.json';
    ...
    private static function generateDump($var, $options) {
        ...
        if(count(\TracyDebugger::getDataValue('dumpPanelTabs')) > 0 && !is_string($var)) {
            $classExt = rand();
            if(($var instanceof \ProcessWire\Wire || $var instanceof \ProcessWire\Wire)) {
                if($var instanceof \ProcessWire\User || $var instanceof \ProcessWire\User) {
                    $type = 'users';
                    $section = 'access';
                }
                elseif($var instanceof \ProcessWire\Role || $var instanceof \ProcessWire\Role) {
                    $type = 'roles';
                    $section = 'access';
                }
                elseif($var instanceof \ProcessWire\Permission || $var instanceof \ProcessWire\Permission) {
                    $type = 'permissions';
                    $section = 'access';
                }
                ...
                elseif($var instanceof \ProcessWire\Page || $var instanceof \ProcessWire\Page) {
                    $type = 'page';
                    $section = '';
                }
                elseif($var instanceof \ProcessWire\Template || $var instanceof \ProcessWire\Template) {
                    $type = 'template';
                    $section = 'setup';
                }
                elseif($var instanceof \ProcessWire\Field || $var instanceof \ProcessWire\Field) {
                    $type = 'field';
                    $section = 'setup';
                }
                elseif($var instanceof \ProcessWire\Module || $var instanceof \ProcessWire\Module) {
                    $type = 'module';
                    $section = '';
                }
            ...
            if($var instanceof \ProcessWire\WireArray || $var instanceof \ProcessWire\WireArray) {
    ...
    private static function humanize($arrayObject) {
        if($arrayObject instanceof \ArrayObject) {
            ...
            if(isset($arrayObject['created_users_id'])) $arrayObject['created_users_id'] = $arrayObject['created_users_id'] . ' (' . \ProcessWire\wire('users')->get($arrayObject['created_users_id'])->name . ')';
            if(isset($arrayObject['modified_users_id'])) $arrayObject['modified_users_id'] = $arrayObject['modified_users_id'] . ' (' . \ProcessWire\wire('users')->get($arrayObject['modified_users_id'])->name . ')';
        }
        return $arrayObject;
    }
